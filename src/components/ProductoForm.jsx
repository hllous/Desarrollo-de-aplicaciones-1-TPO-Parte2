...
const FORM_VACIO = { nombre: '', descripcion: '', precio: '', cantidad: '' };
...
export default function ProductoForm({ productoInicial, onSubmit, onCancelar }) {
	...
	const tituloForm = esEdicion ? 'Editar Producto' : 'Nuevo Producto';
	const textoBoton = esEdicion ? 'Guardar' : 'Agregar';
	...
	return (
		<View style={styles.container}>
			...
			<TextInput
				style={styles.input}
				value={form.nombre}
				onChangeText={(v) => cambiarCampo('nombre', v)}
			...
			<TextInput
				...
				onChangeText={(v) => cambiarCampo('descripcion', v)}
			...
			<TextInput
				style={styles.input}
				value={form.precio}
				onChangeText={(v) => cambiarCampo('precio', v)}
				placeholder="0.00"
			...
			<TextInput
				style={styles.input}
				value={form.cantidad}
				onChangeText={(v) => cambiarCampo('cantidad', v)}
				placeholder="0"
			...
			<View style={styles.botones}>
				...
				<TouchableOpacity style={styles.btnGuardar} onPress={enviar} testID="btn-guardar">
					<Text style={styles.btnGuardarTexto}>{textoBoton}</Text>
				</TouchableOpacity>
			</View>
		</View>
	);
